...
def Green_Line_Tail_start_node(network1,network2,start_nodes):
    ...
    if start_nodes[3] == '3a':
        up = start_nodes[0]
        moves_network_1,moves_network_2,added_node_network_1,added_node_network_2 = GL_Case3_with_start(network1,network2,up,isom_1_2,isom_2_1,start_nodes[1:3])
        if not added_node_network_1:
            return False
    
    if start_nodes[3] == '3bi':
        up = start_nodes[0]
        moves_network_1,moves_network_2,added_node_network_1,added_node_network_2 = GL_Case3_with_start(network1,network2,up,isom_1_2,isom_2_1,start_nodes[1:3])
        if not added_node_network_1:
            return False
    
    if start_nodes[3] == '3bii':
        up = start_nodes[0]
        moves_network_1,moves_network_2,added_node_network_1,added_node_network_2 = GL_Case3_with_start(network1,network2,up,isom_1_2,isom_2_1,start_nodes[1:3])
        if not added_node_network_1:
            return False
    
    if start_nodes[3] == '3biiiA':
        up = start_nodes[0]
        moves_network_1,moves_network_2,added_node_network_1,added_node_network_2 = GL_Case3_with_start(network1,network2,up,isom_1_2,isom_2_1,start_nodes[1:3])
        if not added_node_network_1:
            return False
    
    if start_nodes[3] == '3biiiB':
        up = start_nodes[0]
        moves_network_1,moves_network_2,added_node_network_1,added_node_network_2 = GL_Case3_with_start(network1,network2,up,isom_1_2,isom_2_1,start_nodes[1:3])
        if not added_node_network_1:
            return False
...
#        moves_network_1,moves_network_2,added_node_network_1,added_node_network_2 = GL_Case3(network1,network2,up,isom_1_2,isom_2_1)
    ...
    #Do the green line algorithm
    while(isom_size<goal_size):
        Possible_nodes = NodesConstructor2(network1,network2,isom_1_2,isom_2_1)
        Predicted_node = random.choice(Possible_nodes)
...
def NodesConstructor2(network, target, isom1, isom2):
    List_of_pairs = []
...
#This function will generate networks that have lowest reticulations

def NetworksLowestReticGenerator(n):
    List_of_networks = []
    while len(List_of_networks) < n:
...
#            isom_2_1[node2]=node1
#        
#        Retics_network = LowestReticAndTreeNodeAboveList(Network,isom_1_2)[0]
#        Retics_target = LowestReticAndTreeNodeAboveList(target, isom_2_1)[0]
#        if Retics_network == [] and Retics_target == []:
#            continue
#        else:
#            List_of_networks.append((Network,target))
#This is if we want to use any network, not only that have lowest reticulations
        List_of_networks.append((Network,target))
...
#This generates the node pair list with the min heuristic solution over 100 test runs.        

def DataNodeGenerator(network,target):
    node_list = []
    network = NetworkLeafToLabel(network)
    target = NetworkLeafToLabel(target)
    root1 = Root(network)
    ...
    Retics_network = LowestReticAndTreeNodeAboveList(network, isom_1_2)[0]
    Retics_target = LowestReticAndTreeNodeAboveList(target, isom_2_1)[0]
    for up in Retics_target:
        xp = Child(target,up)
        x = isom_2_1[xp]
        z = Parent(network,x,exclude=isom_1_2.keys())
        if network.in_degree(z) == 2:
            minHeur = RunMinHeur((up,z,None,'1a',network,target))
            node_list.append([(up,z,None,'1a',network,target),minHeur])
        else:
            if CheckMovable(network,(z,x),z):
                Considered_Retics = FindReticList(network,isom_1_2)
                for u in Considered_Retics:
                    minHeur = RunMinHeur((up,z,u,'1bi',network,target))
                    node_list.append([(up,z,u,'1bi',network,target),minHeur])
            else:
                c = Parent(network,z)
                d = Child(network,z,exclude=[x])
                b = Parent(network,c)
                if network.in_degree(b)!=0:
                    ...
                    #First do the move ((c,d),c,(a,b)), then Case1bi applies as (z,x) is now movable
                    newN = DoMove(network,(c,d),c,(a,b))
                    Considered_rets = FindReticList(newN,excludedSet=isom_1_2.keys())
                    for u in Considered_rets:
                        minHeur = RunMinHeur((up,z,u,'1biiA',network,target))
                        node_list.append([(up,z,u,'1biiA',network,target),minHeur])
                if network.in_degree(b) == 0:
                    minHeur = RunMinHeur((up,z,u,'1biiB',network,target))
                    node_list.append([(up,z,None,'1biiB',network,target),minHeur])
    for up in Retics_network:
        xp = Child(network,up)
        x = isom_1_2[xp]
        z = Parent(target,x,exclude=isom_2_1.keys())
        if target.in_degree(z) == 2:
            minHeur = RunMinHeur((up,z,None,'2a',network,target))
            node_list.append([(up,z,None,'2a',network,target),minHeur])
        else:
            if CheckMovable(target,(z,x),z):
                Considered_Retics = FindReticList(target,isom_2_1)
                for u in Considered_Retics:
                    minHeur = RunMinHeur((up,z,u,'2bi',network,target))
                    node_list.append([(up,z,u,'2bi',network,target),minHeur])
            else:
                c = Parent(target,z)
                d = Child(target,z,exclude=[x])
                b = Parent(target,c)
                if network.in_degree(b)!=0:
                    ...
                    #First do the move ((c,d),c,(a,b)), then Case1bi applies as (z,x) is now movable
                    newN = DoMove(target,(c,d),c,(a,b))
                    Considered_rets = FindReticList(newN,excludedSet=isom_2_1.keys())
                    for u in Considered_rets:
                        minHeur = RunMinHeur((up,z,u,'2biiA',network,target))
                        node_list.append([(up,z,u,'2biiA',network,target),minHeur])
                if network.in_degree(b) == 0:
                    minHeur = RunMinHeur((up,z,u,'2biiB',network,target))
                    node_list.append([(up,z,None,'2biiB',network,target),minHeur])
    return node_list

#Second datagenerator also takes into account possible tree nodes

def DataNodeGenerator2(network,target):
    node_list = []
    network = NetworkLeafToLabel(network)
    target = NetworkLeafToLabel(target)
    root1 = Root(network)
    ...
    for label,node1 in label_dict_1.items():
        ...
        isom_2_1[node2]=node1
    Retics_network = LowestReticAndTreeNodeAboveList(network, isom_1_2)[0]
    Retics_target, target_tree_nodes = LowestReticAndTreeNodeAboveList(target, isom_2_1)
    if target_tree_nodes != []:
        for up in target_tree_nodes:
            xp,yp = list(target.successors(up))
            x     = isom_2_1[xp]
            ...
            #Case3a: x and y have a common parent not in the isom
            for parent in common_parents:
                if parent not in isom_1_2.keys():
                    minHeur = RunMinHeur((up,None,None,'3a',network,target))
                    node_list.append([(up,None,None,'3a',network,target),minHeur])
            if list(common_parents) == []:
               z_x = Parent(network,x,exclude=isom_1_2.keys())
               z_y = Parent(network,y,exclude=isom_1_2.keys())
               
               #Case3bi: (z_x,x) is movable                
               if CheckValid(network,(z_x,x),z_x,(z_y,y)):
                   minHeur = RunMinHeur((up,z_x,z_y,'3bi',network,target))
                   node_list.append([(up,z_x,z_y,'3bi',network,target),minHeur])
               #Case3bii: (z_y,y) is movable
               if CheckValid(network,(z_y,y),z_y,(z_x,x)):
                   minHeur = RunMinHeur((up,z_x,z_y,'3bii',network,target))
                   node_list.append([(up,z_x,z_y,'3bii',network,target),minHeur])
               elif Retics_target == [] and Retics_network == []:
                   c_x = Parent(network,z_x)
                   b_x = Parent(network,c_x)
                   ...
                   if network.in_degree(b_x)==0:
                       minHeur = RunMinHeur((up,z_x,z_y,'3biiiB',network,target))
                       node_list.append([(up,z_x,z_y,'3biiiB',network,target),minHeur])
                   else:
                       minHeur = RunMinHeur((up,z_x,z_y,'3biiiA',network,target))
                       node_list.append([(up,z_x,z_y,'3biiiA',network,target),minHeur])
...
def RunMinHeur(nodes):       
    tracker_list = []
    for i in range(0,500):
       try:
           run = Green_Line_Tail_start_node(nodes[4],nodes[5],nodes[0:4])
           tracker_list.append(len(run))
       except:
           pass
    return min(tracker_list)
    
#This was to generate the first data without features: 50 networks, 100 runs of green line.
#7-7-2020: 100 networks and 500 runs of green line
#9-7-2020: 500 networks and 500 runs of green line
#14-7-2020: 500 networks and 500 runs of green line, also taking case 3 into account when generating the data. This results in an extra feature, being the casd 3 feature.
#If we use 1 tree we need case 3 feature else we need to consider different features for two trees.


#test = NetworksLowestReticGenerator(500)
#Data = []
#tracker = 0
#for i in range(0,len(test)):
#    run = DataNodeGenerator2(test[i][0],test[i][1])
#    Data.append(run)
#    tracker +=1
#    print(tracker)
#
#for i in range(len(Data)):
#    Data[i] = sorted(Data[i], key=itemgetter(1))
#np.save('Test20Jul',Data)