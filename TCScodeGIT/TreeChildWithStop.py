...
Data = np.load(r'C:\Users\bryan\Desktop\master thesis\DevelopedData.npy', allow_pickle=True)
...
def setupfunction(Data):
    TotalData = [[] for n in Data]
    global timer
    global monitor
    global Cherries
    global tracker
    ...
    for i in range(len(Data)):
        ...
        Leaves = []
        X = []
        X = [k.name for k in Data[i][0][1][0].get_leaves()]
        Cherries = CherryList(Data[i][0][1])
        for k in range(len(Data[i][0][1])):
            leafset = get_leaves(Data[i][0][1][k],[])
            Leaves.append(leafset)
        for j in range(len(Data[i][2])):
            tracker = 0
            S = Data[i][1]
            Z = copy.deepcopy(S)
            Z.append(Data[i][2][j][0])
            timer = time.time()
            main = TCS(Data[i][0][1],Z,Data[i][0][0]+1,time)
            if main == None and timer == 'timeout':
               SubData.append((Data[i][2][j],tracker,None,False))
            elif main == None and timer != 'timeout':
               SubData.append((Data[i][2][j],tracker,None,True))
            else:
               SubData.append((Data[i][2][j],tracker,len(main)-len(X)))
            monitor +=1
            print(monitor)
        TotalData[i].append((Data[i][0],SubData))
    return TotalData
                
            
test = setupfunction(Data)        
